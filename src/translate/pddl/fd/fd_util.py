...
def default( domain_file, problem_file, output_task1, output_task2 ) :
    parsing_timer = timers.Timer()
    print("Domain: %s Problem: %s"%(domain_file, problem_file) )
    ...
    for atom in atom_names :
        atom_table[ atom ] = index
        output_task1.add_atom( atom.encode('utf-8') )
        output_task2.add_atom( atom.encode('utf-8') )
        index += 1
    ...
    for action in actions :
        ...
        if len(nd_action.negated_conditions) > 0 :
            output_task1.notify_negated_conditions( nd_action.negated_conditions )
            output_task2.notify_negated_conditions( nd_action.negated_conditions )
        nd_actions.append( ( nd_action.name, nd_action ) )

    #NIR: Notify negated atoms in the intial and goal state
    output_task1.notify_negated_conditions(encodeNegatedOnly(task.init, atom_table))
    output_task1.notify_negated_conditions(encodeNegatedOnly(task.goal, atom_table))
    
    #NIR: Notify negated atoms in the intial and goal state
    output_task2.notify_negated_conditions(encodeNegatedOnly(task.init, atom_table))
    output_task2.notify_negated_conditions(encodeNegatedOnly(task.goal, atom_table))
    
    output_task1.create_negated_fluents()
    output_task2.create_negated_fluents()

    for (name, _) in nd_actions :
        output_task2.add_action( name.encode('utf-8'), False )
        output_task1.add_action( name.encode('utf-8'), False )

    index = 0
    for (_,action) in nd_actions :
        output_task2.add_precondition( index, action.precondition )
        output_task1.add_precondition( index, action.precondition )
        for eff in action.effects :
            output_task1.add_effect( index, eff )
            output_task2.add_effect( index, eff )
        #if len(action.cond_effs) != 0 :
        #   print action.name, len(action.cond_effs), "has conditional effects"
        for cond, eff in action.cond_effs.items() :
                        #print( action.name, cond, atom_names[cond[0][0]] )

            output_task2.add_cond_effect( index, list(cond), eff )
            output_task1.add_cond_effect( index, list(cond), eff )
                        
        output_task1.set_cost( index, action.cost ) 
        output_task2.set_cost( index, action.cost ) 
        index += 1
    ...
    for group in mutex_groups :
        if len(group) >= 2 :
            ...
            output_task2.add_mutex_group( encode( group, atom_table ) )
            output_task1.add_mutex_group( encode( group, atom_table ) )
            #print( encode( group, atom_table ) )


    output_task1.set_domain_name( task.domain_name.encode('utf-8') )
    output_task2.set_domain_name( task.domain_name.encode('utf-8') )
    output_task1.set_problem_name( task.task_name.encode('utf-8') )
    output_task2.set_problem_name( task.task_name.encode('utf-8') )
    output_task1.set_init( encode( task.init, atom_table ) )
    output_task2.set_init( encode( task.init, atom_table ) )
    output_task1.set_goal( encode( task.goal, atom_table ) )
    output_task2.set_goal( encode( task.goal, atom_table ) )
    output_task1.parsing_time = parsing_timer.report()
    output_task2.parsing_time = parsing_timer.report()
